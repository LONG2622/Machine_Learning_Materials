...
# ---------------------- 1. 选择聚类特征（你真实的水质指标） ----------------------
cluster_features = ["Temp_C", "DO_ppm", "pH", "Sal"]  # 核心水质指标
df_cluster = df_all[cluster_features].dropna()
...
df_all["水质状态"] = kmeans.fit_predict(data_scaled)

# ---------------------- 4. 给聚类结果起名字（自动判断哪类是清洁/一般/污染） ----------------------
cluster_mean = df_all.groupby("水质状态")[cluster_features].mean()
print("===== 聚类中心（各类别平均指标）=====")
print(cluster_mean.round(2))

# 根据溶解氧DO、盐度、温度自动命名（科学命名）
def name_cluster(row):
    if row["DO_ppm"] > 6.5 and row["Sal"] < 15:
        return "清洁稳定"
    elif row["DO_ppm"] > 4 and row["Sal"] < 25:
        return "一般状态"
    else:
        return "异常/高盐"

cluster_name = cluster_mean.apply(name_cluster, axis=1).to_dict()
df_all["水质状态"] = df_all["水质状态"].map(cluster_name)

# ---------------------- 5. 输出聚类分布（非常有用！） ----------------------
print("\n===== 水质状态聚类分布 =====")
print(df_all["水质状态"].value_counts())

# ---------------------- 6. 聚类可视化（论文图） ----------------------
plt.figure(figsize=(10, 5))
df_all["水质状态"].value_counts().plot(kind="bar", color=["#2E86AB", "#F67280", "#F18F01"])
plt.title("水质状态聚类结果（无监督分类）")
plt.ylabel("样本数量")
plt.tight_layout()
plt.show()

# ---------------------- 7. 散点图：盐度 vs 溶解氧（最核心图） ----------------------
plt.figure(figsize=(9, 6))
for label, color in zip(["清洁稳定", "一般状态", "异常/高盐"], ["blue", "orange", "red"]):
    sub = df_all[df_all["水质状态"] == label]
    plt.scatter(sub["Sal"], sub["DO_ppm"], label=label, alpha=0.5, color=color)

plt.xlabel("盐度 Salinity")
plt.ylabel("溶解氧 DO")
plt.title("水质聚类结果：盐度 vs 溶解氧")
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
plt.show()